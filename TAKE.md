...
ServerMemberManager 启动入口,根据配置启动不同模式(standlone/cluster(file/address))管理Nacos集群Server
...
## 配置中心
最好使用外部存储(Mysql), 那么一次配置直接写入数据库就能保证一个一致性(然后异步通知其他节点从数据库查询写入本地缓存即可完成写入),
并且写入本地文件缓存, 通过 `DumpChangeConfigWorker` 定时对账更新本地缓存, 当客户端查询数据时都是通过本地缓存查询的,目的降低数据库压力
TODO 这里认为一般公司的压力并不需要这个优化？这样的写入也并不是强一致性.目前都是最终一致性？
